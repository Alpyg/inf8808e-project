d3.csv("donn_transf_prop_reqst.csv").then((data) => {
      ...
      x.domain().forEach((time) => {
        svg
          ...
          .on("mouseover", function (event, d) {
            ...
            tooltip
              .html(
                `Région: ${region}<br>Année: ${time}<br>Requete: ${pivotData[region][time]}`
              )
  ...
  const colorOptions = [{ name: "Cool", scale: d3.interpolateCool }];
  ...
  colorSelector.on("change", function (event) {
    ...
    color = d3.scaleSequential(selectedOption.scale).domain([0, zMax]);
